...
def _initialize():
    ...
    global dictAttrToVal
    global dictMsgTypeToVal
...
def stun_test(sock, host, port, source_ip, source_port, send_data=""):
    retVal = {'Resp':False, 'ExternalIP':None, 'ExternalPort':None, 'SourceIP':None, 'SourcePort':None, 'ChangedIP':None, 'ChangedPort':None, 'HostServer':host}
    ...
    while not recvCorr:
        ...
        _tranID = binascii.b2a_hex(buf[4:20]).decode('ascii').upper()

        if dictValToMsgType[ MsgType ] == "BindResponseMsg" and TranID.upper() == _tranID:
            recvCorr = True
            retVal['Resp'] = True
...
def get_nat_type(s, source_ip, source_port, stun_host=None, stun_host_port=3478):
    _initialize()
    port = stun_host_port
    log.debug("Do Test1")
    resp = False
...
def get_ip_info(source_ip="0.0.0.0", source_port=54320, stun_host=None, stun_host_port=3478):
    ...
    nat_type, nat = get_nat_type(s, source_ip, source_port,
                                 stun_host=stun_host,
                                 stun_host_port=stun_host_port)
    external_ip = nat['ExternalIP']
    external_port = nat['ExternalPort']
    host_server = nat['HostServer']
    s.close()
    return (nat_type, external_ip, external_port, host_server)
